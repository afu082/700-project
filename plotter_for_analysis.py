...
def find_initial_ray_equations(angles_degrees, lens_equation):        
    ...
    for angle in angles_degrees:
        # angles neg y neg x (bottom left square) = positive gradient, negative y, positive x = negative gradient
    ...
    for i, initial_ray_equation in enumerate(initial_ray_equations):
        ...
        if(initial_ray_equation[4] == True):
            ...
            if(abs(initial_ray_equation[2]) > abs(initial_ray_equation[3])):
                ...
                # m * start (x-intercept) + c = m2 *  start + c2
...
# read initial angles and relative luminous intensity
input_file_path = "./TestFile1/test2.ies" #relative luminous intensity and angles in IES format, using Typical Spatial Distribution graph of lensless LED from https://docs.rs-online.com/7f9a/0900766b81477f54.pdf
...
def convert(refractive_index_air, refractive_index_lens, equation_inner_lens, equation_outer_lens):
    ...
    # can replace refractive_index_lens to 1 if only one LENS equation is used.
    ...
    while i < len(exit_ray_equations):
        # rays that dont diffract, due to Total internal refraction or hit the wall on the sides of the lens
...
# create maps from angles to candelas
